 ...
 */
public class JacksonStoreIOStreamBenchmark {

    public static final Path PATH = Paths.get("target/tdata/jackson_io_"+ Store.DefaultSize+".json");
    public static final Path PATH_search = Paths.get("target/tdata/jackson_io_search_" + Store.DefaultSize+".json");

    @Benchmark()
    @BenchmarkMode(Mode.Throughput)
    public void loadAndPersist(StringDataSet set) throws IOException {
        new JacksonStoreIOStream(PATH.toString())
                .open()
                .add(set.data())
                .close();
    }
    ...
    public static class JacksonStoreHolder {
        ...
        public void prepare(StringDataSet data) throws IOException {
            store = new JacksonStoreIOStream(PATH_search.toString())
                    .open()
                    .add(data.data())
                    .synchronize();
        }
